...
Any dish can be marked a favourite from its detail screen, and the heart then follows its name wherever that dish is listed in the week — including the other days its category is assigned to. Favourites are kept on the device and are the one thing here the app never generates and can't recreate, so clearing them is its own action in Settings rather than part of clearing generated content.
...
- **Favorites** (`Avocadi/Features/Favorites`) — `FavoritesStore`, an `@Observable` holding the ids of the hearted dishes and persisting them to `UserDefaults` as a plain sorted `[String]`. Deliberately not SwiftData alongside `DishAIContent`, on two counts: `@Query` is a view property, so reading it per dish would mean fetching the whole table inside `DishCategoryView`, a leaf view built fourteen times over a week; and sharing a container with generated content would leave the hearts one careless line away from being deleted by a button whose scope is what the machine wrote. Injected once from `AvocadiApp` with `.environment(_:)` and read straight from the two screens that need it — writing it through `DishDetailViewModel` would make the heart depend on generated content being switched on. The week list draws it inside the dish name's own `Text` rather than beside it, so it flows and wraps with the last word; the comment there explains why that costs a hand-built interpolation. Not shared with the widget, which would need an App Group the project doesn't have.
- **Settings** (`Avocadi/Features/Settings`) — `SettingsView`, the app's only preference screen: a switch for generated content, a destructive action clearing everything `DishAIContentStore` has cached, and a second one clearing favourites, kept apart from it because the two throw away different kinds of thing. `AIContentSettings` holds the `@AppStorage` key and its default in one place, so the screen that writes the setting and the dish screen that reads it can't drift apart on either.
